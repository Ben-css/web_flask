...
document.addEventListener('DOMContentLoaded', function() {
    ...
    selectAllCheckbox.addEventListener('change', function() {
      // 設定所有子 checkbox 的狀態與全選 checkbox 相同
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    if (exportButton) {
        exportButton.addEventListener('click', function () {
            var selectedRows = document.querySelectorAll('.table-body input[type="checkbox"]:checked');
            var idValues = [];

            selectedRows.forEach(function (row) {
                var idValue = row.closest('tr').querySelector('td:nth-child(2)').textContent;
                idValues.push(idValue);
            });

            fetch('/manager/excel_export', {
                method: 'POST',
                headers: {
            ...
            .then(blob => {
                ...
                // 要修改
                downloadLink.download = 'excel_output.xlsx';
